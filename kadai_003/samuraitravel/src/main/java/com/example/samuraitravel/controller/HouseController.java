...
public class HouseController {
	...
	public String show(@PathVariable(name = "id") Integer id, Model model,  @AuthenticationPrincipal UserDetailsImpl userDetailsImpl) {
		...
		List<Review> totalReviews = reviewRepository.findByHouse(house);
		Integer totalReviewCount = totalReviews.size();
		
		
		
		if(userDetailsImpl != null) {
		    User user = userDetailsImpl.getUser();
		    List<Review> hasUserAlreadyReviewed  = reviewRepository.findByHouseAndUser(house, user);
		    List<Favorite> isFavorite = favoriteRepository.findByHouseAndUser(house, user);
		    Favorite favorite = favoriteRepository.findByUserAndHouse(user, house);
		    model.addAttribute("favorite", favorite);
		    model.addAttribute("hasUserAlreadyReviewed", !hasUserAlreadyReviewed.isEmpty());
		    model.addAttribute("isFavorite", !isFavorite.isEmpty());
		} 
